...
const bgImgs = computed(() =>
	Object.fromEntries(
		famílias.value.map((f) => [
			f.Família,
			`bg-[url('/images/${f.exemplares.arquivo}')]`,
		])
	)
);
			...
			<ul class="flex flex-wrap gap-3 justify-around">
				...
				>
					...
					<Image
						...
						class="mx-auto"
						:src="(_familia as Família).exemplares.link ?? '/images/' + (_familia as Família).exemplares.arquivo"
						:alt="_familia.Família"
						width="100"
						preview
					/>
					<p :class="bgImgs[_familia.Família]">
								...
								>
									<div class="md:w-10rem relative">
										<img
											...
											:src="(item as Família).exemplares.link ?? (item as Família).exemplares.arquivo"
											:alt="item.name"
										/>
									</div>
									<div
